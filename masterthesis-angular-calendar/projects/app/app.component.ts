...
import { sources as demoUtilsSources } from './modules/utils/sources';
...
async function getSources(folder: string): Promise<Source[]> {
  const { sources } = await import('./modules/' + folder + '/sources.ts');
...
export class AppComponent implements OnInit {
  ...
  constructor(private router: Router, private http: HttpClient, analytics: Angulartics2GoogleAnalytics) {
...
`.trim(),
    ...
    StackBlitzSDK.openProject(
      ...
      {
        openFile: 'demo/full-calendar.component.ts',
      }
    );
  }
